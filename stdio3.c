...
int main(int argc, char *argv[])
{
  ...
  if (access(FIFO_NAME, F_OK) == -1){
    fifores = mkfifo(FIFO_NAME, 0777);
    if(fifores != 0){
      printf("Could not create FIFO %s \n", FIFO_NAME);
    }
 }

  debug("Process %d opening FIFO", getpid());
  erlpipefd = open(FIFO_NAME, O_WRONLY); 
  debug("Pipe id %d", erlpipefd);
   while((mesglen=read_stdincmd(stdinbuff)) > 0)
     {
	  ...
	  length bytes have been over written */
       ...
       write(erlpipefd,stdinbuff,mesglen);
     }
}
